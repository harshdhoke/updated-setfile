...
exports.updateRow = async (req, res) => {
  const {updates,deletions,insertions } = req.body; // Expecting an array of updates
  console.log("Received batch update request:", updates);
  const insertedResults = [];
  const connection = await pool.getConnection();
  try {
...
exports.cloneFromAny = async (req, res) => {
  const{selectedCustomer,selectedIndexes,table_Nametoupdate,tableData,setfilePrefix}=req.body;
  ...
  try {
      ...
      for (const row of tableData) {
        ...
        if (existing.length > 0) {
         ...
         console.log("N",Tunning_param);
          ...
          const placeholders = insertVals.map(() => "?").join(", ");
          console.log("command",insertCols);
          console.log("valllll",placeholders);
          const [insertResult] = await connection.query(
    ...
    //  res.json({ success: true, message: 'Upsert operation completed successfully' });
    } catch (err) {
      console.error(err);
      res.status(500).json({ error: 'Failed to perform upsert' });
    } 
    await connection.commit();
    await connection.query("UNLOCK TABLES");
